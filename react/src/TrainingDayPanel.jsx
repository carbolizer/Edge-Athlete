...
function ReportPrescription({ prescription, index }) {
  ...
  return <article className="training-report-prescription"><header><span>Effective at day end</span><h5>{prescription.workout_name || prescription.workout?.name || prescription.name || (prescription.source === "legacy" ? "Legacy prescription" : `Workout ${index + 1}`)}</h5></header>{exercises.map((exercise, exerciseIndex) => {
    return <section key={exercise.id || exercise.position || exerciseIndex}><div className="training-report-target"><div><span>{exercise.position ?? exerciseIndex + 1}. {exercise.exercise || exercise.name || "Exercise"}</span><b>{reportValue(exercise.sets ?? exercise.target_sets)} sets x {reportValue(exercise.reps ?? exercise.target_reps)} reps</b></div><div><span>Target load</span><b>{reportValue(exercise.default_weight_lbs ?? exercise.weight_lbs ?? exercise.target_weight_lbs, " lbs")}</b></div><div><span>Velocity</span><b>{exercise.velocity_min === null || exercise.velocity_min === undefined ? "--" : `${reportValue(exercise.velocity_min)}-${reportValue(exercise.velocity_max)} m/s`}</b></div></div></section>;
  })}</article>;
}
