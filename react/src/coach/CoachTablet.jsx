 ...
 * --------------------------------
 * Extra coach page for Room Layout assignment: JWT login gate, then a
 * dropdown-and-assign UI (not drag-and-drop) that wires unassigned rack screens
 * and nodes into numbered rack slots via the existing coach PATCH endpoints.
 * Group / block / session drill-down stay out of scope.
 */
...
function useCoachIdentity() {
  useEffect(() => {
    ...
    document.title = 'Edge Athlete — Coach'

    const icon = document.createElement('link')
    icon.rel = 'icon'
    icon.type = 'image/svg+xml'
    icon.href = '/coach-icon.svg'
    document.head.appendChild(icon)

    const manifest = document.createElement('link')
    manifest.rel = 'manifest'
    manifest.href = '/manifest.coach.json'
...
function AssignRow({
  ...
  return (
    <div>
      <h3 style={{ margin: '0 0 10px', fontSize: 14, color: 'var(--coach-ink)' }}>{label}</h3>
...
function RoomLayout({ token, onAuthLost }) {
  ...
  // No GET-all-screens API yet — remember coach assigns from this session for the grid.
  ...
  // Nodes with no rack (or all nodes for reassignment) — show unassigned first, then assigned.
  ...
  return (
    <section className="coach-card">
      ...
      <hr className="coach-divider" />
      <h3 style={{ margin: '0 0 6px', fontSize: 14 }}>Rack slots</h3>
    ...
    <div className="coach-root">
      <div className="coach-shell">
        <div className="coach-topbar">
          <div>
            <p className="coach-eyebrow">Edge Athlete · Coach</p>
            <h1 className="coach-brand">Edge Athlete</h1>
