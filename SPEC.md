...
**Room Layout (assignment):** one section of the consolidated view — a grid of rack slots (1..N) plus two source pools: "Unassigned Screens" (`GET /api/racks/unassigned/`, each shown by its short device id — the same id that rack screen displays on its own "waiting for assignment" state) and nodes available for reassignment (`GET /api/nodes/`). Assigning a screen to a rack slot calls `PATCH /api/racks/{device_id}/` with the rack number; assigning a node calls `PATCH /api/nodes/{node_id}/` from Phase 4. **Demo cut:** the shipped `/coach` Room Layout uses dropdown-and-assign (not drag-and-drop); shared DnD component and group/block/session drill-down stay out of scope for that cut.
