"""Formats stored DailyReport snapshots into the shapes the reports UI reads.

READ-ONLY, and deliberately paranoid. Everything here works off the frozen JSON
snapshot rather than live tables, and every accessor tolerates a missing or
malformed piece (see the _object/_value/_items helpers). That matters because a
snapshot written months ago by an older version of the code must still render
today without blowing up — so absent fields degrade to null instead of raising.

Adopted from Braydon's branch essentially unchanged: it depends only on the
snapshot shape and DailyReport, not on any of the tables the merge dropped.
"""
