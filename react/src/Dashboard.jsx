...
function CoachView({ monitor, accessToken, onLogout }) {
  ...
  const chooseAthlete=id=>{if(dirty&&!window.confirm("Discard the unsaved note draft?"))return;if(window.__edgePlanningDirty?.schedule&&!window.confirm("Discard the unsaved athlete schedule changes?"))return;setSelectedAthleteId(id?Number(id):null);};
  const chooseTab=tab=>{if(activeTab==="notes"&&tab!=="notes"&&dirty&&!window.confirm("Leave Notes with unsaved changes?"))return;if(activeTab==="workouts"&&tab!=="workouts"&&window.__edgePlanningDirty?.catalog&&!window.confirm("Leave workout planning with an unsaved draft?"))return;if(activeTab==="programs"&&tab!=="programs"&&window.__edgePlanningDirty?.schedule&&!window.confirm("Leave athlete scheduling and discard unsaved changes?"))return;setActiveTab(tab);};
