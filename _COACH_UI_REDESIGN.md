...
|---|---|---|---|
| **A** | The shell | Three routes, the glass navbar, the sliding pill. Each state shows the old tabs unchanged | ✅ `d9cb37f` |
| **B** | SESSION | The live room + starting a day. Split `TrainingDayPanel`: open-from-scratch → PLANNING, start-staged → SESSION | ✅ `ba114cd` |
...
| **C** | ANALYTICS | Athlete selector moves here from the topbar; one "choose an athlete" guard instead of four | ✅ `bce0993` |
...
| **G** | Dashboard Settings | A cog on the *wall* display holding Change device role. Genuinely new; may be its own branch | ✅ `71f2d3f` |
...
| | Commit |
|---|---|
| "Rack N is ready" deleted | `75439e8` |
| "No racks assigned" deleted; cog labelled **⚙ ROOM LAYOUT** | `f3e535e` |
| Log out moved under the logo; Change device removed from the topbar | `75439e8` |
| Four "Choose an athlete" guards collapsed to one | `75439e8` |
| Summary strip hidden behind `isDevMode()` | `ba50b31` |
| `ProgramsTab` null-key + exercise-id-as-name bugs fixed | `e53a8a1` |
...
| 5 | 330 | "Choose an athlete" | Coach · athlete | ✅ **Collapsed** — `75439e8` |
| 6 | 386 | "Choose an athlete" | Coach · history | ✅ **Collapsed** — `75439e8` |
| 7 | 391 | "No completed training days" | Coach · history | ✅ Keep — real empty state, distinct from #6 |
| 8 | 403 | "Choose an athlete" | Coach · programs | ✅ **Collapsed** — `75439e8` |
| 9 | 411 | "Choose an athlete" | Coach · notes | ✅ **Collapsed** — `75439e8` |
...
| 12 | 491 | "No racks assigned" | Coach · room | ✅ **Deleted** — `f3e535e`; the cog carries it instead |
| 13 | 510 | "Loading athlete context" | Coach | ✅ Keep |
| 14 | 510 | "Athlete context unavailable" | Coach | ✅ Keep |
| — | 495 | "Rack N is ready" | Coach · room | ✅ **Deleted** — `75439e8` |
...
| | | |
|---|---|---|
| **D1** | The four sub-tabs, Groups view, Calendar renamed | ✅ `3a20863` |
| **D2** | The calendar's card view | ✅ `2e4dc79` |
| **D3** | Design tab surgery — dropdowns, in-component deploy/promote | ✅ `ad805b8` |
...
- [x] Stage a day → lands in SESSION — **done in Phase B** (`ba114cd`)
...
**Phase E — the widget** — ✅ **DONE, absorbed into Phase B** (`ba114cd`)
...
8. `ProgramsTab` null-key bug? → **Fixed**, `e53a8a1`
...
- **2026-07-30** — **Programs tab: two bugs fixed** (outside this doc, commit
  `e53a8a1`). Both had one root cause — it is Braydon's component, carried in by
...
- **2026-07-31** — **Phase B built** (`ba114cd`). `TrainingDayPanel` split in two:
