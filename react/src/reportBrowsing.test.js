...
describe("report browsing pages", () => {
  ...
  it("builds ID-only daily and athlete PDF downloads", () => {
    ...
    // The athlete's copy is the same report filtered, not a parallel
    // athletes/{id}/reports/... route (merge canon R6). The filename still says
    // whose copy it is, because that is what lands in their downloads folder.
    expect(reportPdfDownload("athlete", 12, 7)).toEqual({
      url: "/api/reports/12/pdf/?athlete=7",
