"""Turning a reusable template into a squad's actual plan.

THE ONE IDEA
A coach writes a training block once — "Day 1: squat 5x3 at 80%, bench 3x5 at
75%" — and redeploys it for years. Deploying it for a squad copies those rows
down into a program that belongs to that squad and starts on a date.

WHY COPY INSTEAD OF POINTING AT THE TEMPLATE
Because a coach will tweak. If the program just referenced the template, then
editing the template next season would silently rewrite what last season's squad
...
squad changes; either way, what already happened stays true.
"""
...
def instantiate_block(block, group, name=None, start_date=None, end_date=None):
    """Deploy a template for a squad. Returns the new program.

    All-or-nothing: a program that only got half its workouts copied would look
    complete to a coach and short-change the squad mid-week, so a failure part
