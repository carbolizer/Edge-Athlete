...
:::::{dropdown} `Node` — An ESP32 + sensor unit on a rack
...
:::::{dropdown} `RackScreen` — A tablet PWA standing at a rack
...
:::::{dropdown} `Tag` — A label for grouping movements (e.g
...
:::::{dropdown} `Exercise` — The catalog entry for one movement — the single official identity that every training plan, set, and reference max points at, instead of each one hand-typing the name
...
:::::{dropdown} `TrainingGroup` — A NAMED SUBSET of athletes who train together on one program — e.g
...
:::::{dropdown} `TrainingGroupCoach` — Which staff run a TrainingGroup, and in what capacity
...
:::::{dropdown} `BlockCategory` — A label for finding things in the shared block catalog — "Off-season", "Football", "Freshman"
...
:::::{dropdown} `TrainingBlock` — A reusable, timeless TEMPLATE a coach designs once and redeploys (tweak last year's block, run it again next year)
...
:::::{dropdown} `TrainingBlockExercise` — One prescription row inside a block workout — the MASTER copy a program snapshot-copies from at instantiation
...
:::::{dropdown} `TrainingProgram` — A scheduled INSTANCE for a group, placed in time
...
:::::{dropdown} `TrainingProgramWorkout` — The editable copy of a TrainingBlockWorkout, living on this program instance
...
:::::{dropdown} `ScheduledSession` — One planned training slot: this program's day N, on this date
...
:::::{dropdown} `TrainingProgramExercise` — The editable copy of a TrainingBlockExercise — the runtime prescription row
...
:::::{dropdown} `AthleteWorkoutExerciseOverride` — A coach-set per-athlete EXCEPTION for one prescription row — for the rare outlier where a percent doesn't fit that specific athlete
...
:::::{dropdown} `TrainingSession` — One training session in the gym — a window of time containing many sets across the athletes who took part
...
:::::{dropdown} `SessionParticipation` — The join between a shared session and one group's program — this is what lets many groups be on the same session at once instead of a session belonging to just one group
...
:::::{dropdown} `Set` — One set an athlete performed
...
:::::{dropdown} `AthleteReferenceMax` — An athlete's CURRENT WORKING reference for one movement — the anchor number programs multiply against to build target weights
...
:::::{dropdown} `Rep` — One completed rep inside a set
...
:::::{dropdown} `RackCheckIn` — A record that an athlete signed in ("checked in") at a rack during a session
...
:::::{dropdown} `DailyReport` — The permanent record of one finished training day — written once, never edited
...
:::::{dropdown} `MonitoringEvent` — A durable record that "something changed" — written the instant it happens; a separate publisher loop delivers it to the dashboard afterward and marks it published
